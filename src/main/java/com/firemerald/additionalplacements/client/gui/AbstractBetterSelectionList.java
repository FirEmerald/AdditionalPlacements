...
public abstract class AbstractBetterSelectionList<E extends AbstractBetterSelectionList.Entry<E>> extends FocusableGui implements IRenderable {
	...
	protected final E getEntryAtPosition(double mouseX, double mouseY) {
		...
		int mousePosY = MathHelper.floor(mouseY - this.getY()) - this.headerHeight + (int) this.getScrollAmount() - 4;
	...
	protected void renderHeader(MatrixStack poseStack, int x, int y) {
	}
	...
	public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		if (maxScroll > 0) {
			...
			scrollSize = MathHelper.clamp(scrollSize, 32, this.height - 8);
	...
	public void setScrollAmount(double scroll) {
		this.scrollAmount = MathHelper.clamp(scroll, 0.0D, this.getMaxScroll());
	}
	...
	protected boolean moveSelection(SelectionDirection direction, Predicate<E> validator) {
		...
		if (!this.children().isEmpty()) {
			...
			while(true) {
				int nextIndex = MathHelper.clamp(curIndex + dir, 0, this.getItemCount() - 1);
	...
	protected void renderList(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
	...
	protected void renderItem(MatrixStack poseStack, int mouseX, int mouseY, float partialTick, E entry, int left, int top, int width, int height) {
	...
	protected void renderSelection(MatrixStack poseStack, int top, int width, int height, int outerColor, int innerColor) {
	...
	protected abstract static class Entry<E extends AbstractBetterSelectionList.Entry<E>> implements IGuiEventListener {
		/** @deprecated */
		@Deprecated
		...
		public abstract void render(MatrixStack poseStack, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTick);

		public void renderBack(MatrixStack poseStack, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTick) {
		}
