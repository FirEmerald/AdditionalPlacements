...
public class BlockModelUtils
{
	...
	public static Pair<TextureAtlasSprite, Integer> getSidedTexture(BlockState fromState, BakedModel fromModel, Direction fromSide, Random rand, int vertexSize, int posOffset) {
	...
	public static List<BakedQuad> retexturedQuads(BlockState modelState, BakedModel originalModel, BakedModel ourModel, Direction side, Random rand)
	{
	...
	public static List<BakedQuad> retexturedQuads(BlockState state, BlockState modelState, Function<BlockState, BakedModel> originalModel, BakedModel ourModel, Direction side, Random rand) {
		return retexturedQuads(modelState, originalModel.apply(modelState), ourModel, side, rand);
	}

	public static List<BakedQuad> rotatedQuads(BlockState modelState, BakedModel model, BlockRotation rotation, boolean rotateTex, Direction side, Random rand)
	{
		VertexFormat format = DefaultVertexFormat.BLOCK;
		int vertexSize = format.getIntegerSize();
		int posOffset = ((IVertexFormatExtensions) format).getIntOffset(DefaultVertexFormat.ELEMENT_POSITION);
	...
	public static List<BakedQuad> rotatedQuads(BlockState modelState, Function<BlockState, BakedModel> model, BlockRotation rotation, boolean rotateTex, Direction side, Random rand) {
		BakedModel originalModel = model.apply(modelState);
		return rotatedQuads(modelState, originalModel, rotation, rotateTex, side, rand);
	}

	public static List<BakedQuad> rotateQuads(BlockState modelState, BakedModel model, BlockRotation rotation, boolean rotateTex, Direction side, Random rand)
	{
		VertexFormat format = DefaultVertexFormat.BLOCK;
		int vertexSize = format.getIntegerSize();
		int posOffset = ((IVertexFormatExtensions) format).getIntOffset(DefaultVertexFormat.ELEMENT_POSITION);
