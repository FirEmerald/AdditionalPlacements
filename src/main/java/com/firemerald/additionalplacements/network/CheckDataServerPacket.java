...
public class CheckDataServerPacket extends ServerPacket
{
	...
	public void handleServer(NetworkEvent.Context context)
	{
		...
		if (!compiledErrors.isEmpty()) {
			...
			context.getNetworkManager().disconnect(Component.translatableWithFallback("msg.additionalplacements.disconnected", "Additional Placements configuration conflict"));
		}
	}
}