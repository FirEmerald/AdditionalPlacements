...
public class ConfigurationCheckFailedPacket extends ClientLoginPacket
{
	...
	public void handle(NetworkEvent.Context context)
	{
		...
		MessageTree rootError = new MessageTree(new TranslatableComponent("msg.additionalplacements.errors.type"));
		compiledErrors.forEach(data -> {
			MessageTree typeError = new MessageTree(new TextComponent(data.getLeft().toString())); //TODO friendly name?
			List<MessageTree> clientErrors = data.getMiddle();
			if (!clientErrors.isEmpty()) {
				MessageTree clientError = new MessageTree(new TranslatableComponent("msg.additionalplacements.errors.client"), clientErrors);
				typeError.children.add(clientError);
			}
			List<MessageTree> serverErrors = data.getRight();
			if (!serverErrors.isEmpty()) {
				MessageTree serverError = new MessageTree(new TranslatableComponent("msg.additionalplacements.errors.server"), serverErrors);
